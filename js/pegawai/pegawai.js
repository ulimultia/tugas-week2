...
function filter() {
    let devisi = document.getElementById("filter-devisi").value;
    let jabatan = document.getElementById("filter-jabatan").value;
    // console.log(devisi, jabatan);
    if (devisi === '' && jabatan === '') {
        Swal.fire({
            icon: 'error',
            title: 'Gagal!!!',
        ...
        fetch('./assets/data/pegawai.json')
            ...
            .then(function(employees) {
                showFilterResult(employees, devisi, jabatan);
            })
    }
}

function showFilterResult(data, devisi, jabatan) {
    // console.log(jabatan, devisi);
    let print = '';
    for (let i = 0; i < data.length; i++) {
        if (data[i].devisi == devisi && data[i].jabatan == jabatan) {
            print += `
                    <tr>
                ...
                `
        } else if (data[i].devisi == devisi && jabatan == '') {
            print += `
                    <tr>
                        <td class="text-center">${i+1}</td>
                        <td>${data[i].nama}</td>
                ...
                `
        } else if (devisi === '' && data[i].jabatan == jabatan) {
            print += `
                    <tr>
                        <td class="text-center">${i+1}</td>
                        <td>${data[i].nama}</td>
